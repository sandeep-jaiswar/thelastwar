 ...
 * Using integer constants instead of enums to avoid autoboxing and improve
 * performance.
 */
public final class EventType {
    ...
    public static boolean isValid(int eventType) {
        // Check for negative values first for better performance
        if (eventType < 0) {
            return false;
        }
    ...
    public static String getSubsystem(int eventType) {
        if (eventType >= 1000 && eventType < 2000)
            return "FeedHandler";
        if (eventType >= 2000 && eventType < 3000)
            return "MatchingEngine";
        if (eventType >= 3000 && eventType < 4000)
            return "Risk";
        if (eventType >= 4000 && eventType < 5000)
            return "OMS";
        if (eventType >= 5000 && eventType < 6000)
            return "Analytics";
        if (eventType >= 9000 && eventType < 10000)
            return "System";
        return "Unknown";
    }
}
