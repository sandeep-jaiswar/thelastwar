...
class OrderRequestTest {
    ...
    void testInvalidSide() {
        OrderRequest request = new OrderRequest("AAPL", "INVALID", "LIMIT", 100, 15000, 999);
        assertThrows(IllegalArgumentException.class, request::validate);
    }
    
    @Test
    void testInvalidQuantity() {
        OrderRequest request = new OrderRequest("AAPL", "BUY", "LIMIT", 0, 15000, 999);
        assertThrows(IllegalArgumentException.class, request::validate);
    }
    
    @Test
    void testMarketOrderWithZeroPrice() {
        OrderRequest request = new OrderRequest("AAPL", "BUY", "MARKET", 100, 0, 999);
        assertDoesNotThrow(request::validate);
    }
    
    @Test
    void testLimitOrderWithZeroPrice() {
        OrderRequest request = new OrderRequest("AAPL", "BUY", "LIMIT", 100, 0, 999);
        assertThrows(IllegalArgumentException.class, request::validate);
    }
    
    @Test
    void testSellOrder() {
        OrderRequest request = new OrderRequest("AAPL", "SELL", "LIMIT", 100, 15000, 999);
        assertEquals((byte) 2, request.getSideByte());
    }
    
    @Test
    void testOrderTypes() {
        OrderRequest market = new OrderRequest("AAPL", "BUY", "MARKET", 100, 0, 999);
        assertEquals((byte) 1, market.getOrderTypeByte());
        
        OrderRequest limit = new OrderRequest("AAPL", "BUY", "LIMIT", 100, 15000, 999);
        assertEquals((byte) 2, limit.getOrderTypeByte());
        
        OrderRequest stop = new OrderRequest("AAPL", "BUY", "STOP", 100, 15000, 999);
        assertEquals((byte) 3, stop.getOrderTypeByte());
        
        OrderRequest stopLimit = new OrderRequest("AAPL", "BUY", "STOP_LIMIT", 100, 15000, 999);
