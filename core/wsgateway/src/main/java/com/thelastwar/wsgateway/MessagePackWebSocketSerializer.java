...
public class MessagePackWebSocketSerializer implements WebSocketMessageSerializer {
    ...
    public byte[] serialize(Event event) throws SerializationException {
        try (MessageBufferPacker packer = MessagePack.newDefaultBufferPacker()) {
            ...
            packer.packString(KEY_TIMESTAMP);
            ...
            packer.packString(KEY_PAYLOAD);
            Object payload = event.payload();
            if (payload instanceof OrderEvent ord) {
                packOrderEvent(packer, ord);
            } else if (payload instanceof ExecutionEvent exec) {
                packExecutionEvent(packer, exec);
            } else {
                packer.packNil();
            }
    ...
    public WebSocketMessage deserialize(byte[] data) throws SerializationException {
        try (MessageUnpacker unpacker = MessagePack.newDefaultUnpacker(data)) {
            ...
            for (int i = 0; i < mapSize; i++) {
                ...
                } else if (KEY_PAYLOAD.equals(key)) {
